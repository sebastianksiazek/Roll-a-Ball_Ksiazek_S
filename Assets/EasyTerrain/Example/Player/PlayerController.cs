    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
			...
			if (transform.position.y < terrainSampleAtPlayer.height + minHeightToGround)
            {
                ...
                rb.linearVelocity = Vector3.RotateTowards(rb.linearVelocity, Vector3.ProjectOnPlane(rb.linearVelocity, terrainSampleAtPlayer.normal) * rb.linearVelocity.magnitude, Mathf.PI * 0.5f * deltaTime, 1f * deltaTime);
            }
            else
            {
                rb.linearVelocity = Vector3.RotateTowards(rb.linearVelocity, transform.TransformDirection(inputWASD) * rb.linearVelocity.magnitude, Mathf.PI * 0.5f * deltaTime, 1f * deltaTime);
            }
            ...
            if (rb.linearVelocity.magnitude > maxSpeed * speedMultiplierTemp)
            {
                speed = Mathf.Lerp(rb.linearVelocity.magnitude, maxSpeed * speedMultiplierTemp, smoothFactor);
